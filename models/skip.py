...
def skip(
    ...
    for i in range(len(num_channels_down)):
        ...
        if num_channels_skip[i] != 0:
            skip.add(conv(input_depth, num_channels_skip[i], filter_skip_size, bias=need_bias, pad=pad,
                          dropout_mode=dropout_mode_skip, dropout_p=dropout_p_skip, iterator=module_name_iter_skip, string='skip'))
        ...
        deeper.add(conv(input_depth, num_channels_down[i], filter_size_down[i], 2, bias=need_bias, pad=pad,
                        downsample_mode=downsample_mode[i], dropout_mode=dropout_mode_down, dropout_p=dropout_p_down,
                        iterator=module_name_iter_deeper, string='deeper'))
        ...
        deeper.add(conv(num_channels_down[i], num_channels_down[i], filter_size_down[i], bias=need_bias, pad=pad,
                        dropout_mode=dropout_mode_down, dropout_p=dropout_p_down, iterator=module_name_iter_deeper, string='deeper'))
        deeper.add(bn(num_channels_down[i]))
        deeper.add(act(act_fun))
        ...
        model_tmp.add(conv(num_channels_skip[i] + k, num_channels_up[i], filter_size_up[i], 1, bias=need_bias, pad=pad,
                           ...
                           iterator=module_name_iter_up-1, string='up'))
        ...
        if need1x1_up:
            model_tmp.add(conv(num_channels_up[i], num_channels_up[i], 1, bias=need_bias, pad=pad,
                               dropout_mode=dropout_mode_up, dropout_p=dropout_p_up,
                               iterator=module_name_iter_up, string='up'))
            model_tmp.add(bn(num_channels_up[i]))
            model_tmp.add(act(act_fun))
    ...
    model.add(conv(num_channels_up[0], num_output_channels, 1, bias=need_bias, pad=pad,
                   dropout_mode=dropout_mode_output, dropout_p=dropout_p_output, iterator=final_iter, string='up'))
    if need_sigmoid:
